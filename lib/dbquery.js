...
var api_keys = {
    'bXPb2ppk7ktvZLy3tafDxxx8MjeVHmVRCu4sDR407A79O6y2YxIimuMvzxdCaSs1': {
        'first_name': 'Andy',
        'last_name': 'Dvorak',
        'email': 'acdvorak@gmail.com',
        'role': 'admin'
    },
    'FQfArZ7jaS1lQRekhS94W0JDZ33Nbd5WV8AJaK22xwkOhi7sFffX5cFjR18AE3m2': {
        'first_name': 'Chris',
        'last_name': 'Spencer',
        'email': 'chrisflyer@gmail.com',
        'role': 'admin'
    }
};

function DBQuery(req, params, body) {
    this.req = req;
    this.params = params || {};
    this.body = body || {};
    this._callback = null;
...
DBQuery.prototype.post = function(callback) {
    ...
    if ( ! api_keys[this.params.api_key] ) {
        this._callback(
            {
                'textStatus':'invalid_api_key',
    ...
    dbManager.connect((function(err, db) {
        ...
        db.collection('discs', (function(err, collection) {
            ...
            console.log('POST JSON: ', JSON.parse(this.body.json));

            var discJson = this.body.json ? JSON.parse(this.body.json) : null;
            ...
            if (discJson) {
                ...
                discJson['submitter'] = {
                    ...
                    'api_key': obj.params.api_key
                };

                console.log('discJson: ', discJson);

                collection.insert(discJson, { safe: true }, function(err, result) {
                    console.log('Insert err = ', err);
                    console.log('Insert result = ', result);
                    if ( jQuery.isFunction(obj._callback) ) {
                        obj._callback(err, result);
                    } else {
                    }
                });
            }
...
DBQuery.Query = function(req, params, callback) {
    new DBQuery(req, params).query(callback);
};

DBQuery.Post = function(req, params, body, callback) {
    new DBQuery(req, params, body).post(callback);
};
